...
public class SchoolDbContext(DbContextOptions<SchoolDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Payment>(buildAction =>
        {
            ...
            buildAction.Property(p => p.Method).HasConversion<string>().HasColumnType("varchar(20)").IsRequired();
            ...
            buildAction.Property(p => p.Status).HasConversion<string>().HasColumnType("varchar(20)").IsRequired();
            ...
            buildAction.ToTable("Payments", t =>
            {
                ...
                t.HasCheckConstraint("CHK_METHOD_VALID", "[Method] IN ('Cash', 'BankTransfer', 'QR')");
        ...
        modelBuilder.Entity<Schedule>(buildAction =>
        {
            ...
            // Nullable — Free Period rows carry no subject or teacher
            buildAction.Property(s => s.SubjectId).HasColumnType("varchar(50)").IsRequired(false);
            buildAction.Property(s => s.TeacherId).HasColumnType("varchar(50)").IsRequired(false);

            buildAction.Property(s => s.Day).HasConversion<string>().HasMaxLength(20).IsRequired();
            ...
            buildAction.Property(s => s.IsFreePeriod).HasDefaultValue(false);
            buildAction.Property(s => s.IsAutoGenerated).HasDefaultValue(false);
            buildAction.Property(s => s.CreatedAt).HasColumnType("datetime2").IsRequired();
            buildAction.Property(s => s.UpdatedAt).HasColumnType("datetime2").IsRequired(false);

            // One class cannot have two subjects in the same slot (Free Periods included —
            // they still occupy the cell, so this index stays unfiltered)
            buildAction.HasIndex(s => new { s.ClassId, s.Day, s.StartTime })
                       .IsUnique()
                       .HasDatabaseName("UQ_Class_Day_StartTime");

            // A teacher cannot be in two classes at the same day+time.
            // Filter excludes Free Period rows (TeacherId IS NULL) — SQL Server ignores NULLs
            // in unique indexes by default, but being explicit avoids any edge-case surprises.
            buildAction.HasIndex(s => new { s.TeacherId, s.Day, s.StartTime })
                       .IsUnique()
                       .HasFilter("[TeacherId] IS NOT NULL AND [IsFreePeriod] = 0")
                       .HasDatabaseName("UQ_Teacher_Day_StartTime");
            ...
            // No cascade on Subject/Teacher — deleting a subject or teacher should be
            // blocked if they are still referenced in a schedule, not silently wiped.
            ...
            {
                t.HasCheckConstraint(
                    "CHK_DAYOFWEEK_NOT_EMPTY", 
                    "LEN(LTRIM(RTRIM([Day]))) > 0"
                );
                t.HasCheckConstraint(
                ...
                // Free Period rows must not reference a subject or teacher
                t.HasCheckConstraint("CHK_FREEPERIOD_NO_SUBJECT",
                    "[IsFreePeriod] = 0 OR [SubjectId] IS NULL");
                t.HasCheckConstraint("CHK_FREEPERIOD_NO_TEACHER",
                    "[IsFreePeriod] = 0 OR [TeacherId] IS NULL");

                // Real subject rows must have both a subject and a teacher
                t.HasCheckConstraint("CHK_SUBJECT_REQUIRES_TEACHER",
                    "[IsFreePeriod] = 1 OR ([SubjectId] IS NOT NULL AND [TeacherId] IS NOT NULL)");
            });
        });
        #endregion
        #region Enrollments
