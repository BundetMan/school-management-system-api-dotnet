...
namespace SchoolAPI.Repositories
{
    public class StudentRepository(SchoolDbContext dbContext) : IStudentRepository
    {
        ...
        public async Task<Student?> GetByCodeWithDetailsAsync(string code)
        ...
        public async Task<Student?> SearchWithDetailsAsync(Expression<Func<Student, bool>> predicate)
            => await GetQueryableWithDetails()
        ...
        public async Task<(List<Student> items, int totalCount)> GetPageWithDetailsAsync(int page = 1, int pageSize = 30)
        {
            if (page <= 0) page = 1;
            if (pageSize <= 0) pageSize = 30;
            var query = GetQueryableWithDetails().AsNoTracking();
